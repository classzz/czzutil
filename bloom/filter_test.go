...
func TestFilterInsertKey(t *testing.T) {
	...
	wif, err := czzutil.DecodeWIF(secret)
	if err != nil {
	...
	f.Add(czzutil.Hash160(wif.SerializePubKey()))
...
func TestFilterBloomMatch(t *testing.T) {
	...
	tx, err := czzutil.NewTxFromBytes(strBytes)
	if err != nil {
	...
	spendingTx, err := czzutil.NewTxFromBytes(spendingTxBytes)
	if err != nil {
		t.Errorf("TestFilterBloomMatch NewTxFromBytes failure: %v", err)
		return
	}
