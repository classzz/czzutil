...
//
// This method is safe for concurrent access.
func (s *utxoCache) FetchTxView(tx *czzutil.Tx) (*UtxoViewpoint, error) {
	s.mtx.Lock()
	defer s.mtx.Unlock()
...
func (b *BlockChain) FetchUtxoView(tx *czzutil.Tx) (*UtxoViewpoint, error) {
	b.chainLock.RLock()
	defer b.chainLock.RUnlock()
...
//
// This method should be called with the state lock held.
func (s *utxoCache) rollBackBlock(block *czzutil.Block, stxos []SpentTxOut) error {
...
//
// This method should be called with the state lock held.
func (s *utxoCache) rollForwardBlock(block *czzutil.Block) error {
