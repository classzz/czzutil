...
// These constants are the values of the official opcodes used on the czz wiki,
// in bitcoin core and in most if not all other references and software related
// to handling CZZ scripts.
const (
	OP_0                   = 0x00 // 0
...
func opcodeCheckSig(op *parsedOpcode, vm *Engine) error {
	...
	pubKey, err := czzec.ParsePubKey(pkBytes, czzec.S256())
	if err != nil {
		vm.dstack.PushBool(false)
		return nil
	}

	var signature *czzec.Signature
	if vm.hasFlag(ScriptVerifySchnorr) && len(sigBytes) == 64 {
		signature, err = czzec.ParseSchnorrSignature(sigBytes)
	} else if vm.hasFlag(ScriptVerifyStrictEncoding) ||
		vm.hasFlag(ScriptVerifyDERSignatures) {

		signature, err = czzec.ParseDERSignature(sigBytes, czzec.S256())
	} else {
		signature, err = czzec.ParseBERSignature(sigBytes, czzec.S256())
	}
	if err != nil {
		vm.dstack.PushBool(false)
		return nil
...
type parsedSigInfo struct {
	signature       []byte
	parsedSignature *czzec.Signature
	parsed          bool
}
	...
	for numSignatures > 0 {
		...
		if !sigInfo.parsed {
			...
			if vm.hasFlag(ScriptVerifyStrictEncoding) ||
				...
				parsedSig, err = czzec.ParseDERSignature(signature,
					czzec.S256())
			} else {
				parsedSig, err = czzec.ParseBERSignature(signature,
					czzec.S256())
			}
			sigInfo.parsed = true
			if err != nil {
		...
		// Parse the pubkey.
		parsedPubKey, err := czzec.ParsePubKey(pubKey, czzec.S256())
		if err != nil {
			continue
		}
