...
func (cs *CoinSet) TotalValue() (value czzutil.Amount) {
...
func satisfiesTargetValue(targetValue, minChange, totalValue czzutil.Amount) bool {
...
type CoinSelector interface {
	CoinSelect(targetValue czzutil.Amount, coins []Coin) (Coins, error)
}
...
type MinIndexCoinSelector struct {
	MaxInputs       int
	MinChangeAmount czzutil.Amount
}

// CoinSelect will attempt to select coins using the algorithm described
// in the MinIndexCoinSelector struct.
func (s MinIndexCoinSelector) CoinSelect(targetValue czzutil.Amount, coins []Coin) (Coins, error) {
	cs := NewCoinSet(nil)
...
func (s MinNumberCoinSelector) CoinSelect(targetValue czzutil.Amount, coins []Coin) (Coins, error) {
...
func (s MaxValueAgeCoinSelector) CoinSelect(targetValue czzutil.Amount, coins []Coin) (Coins, error) {
	sortedCoins := make([]Coin, 0, len(coins))
	sortedCoins = append(sortedCoins, coins...)
...
func (s MinPriorityCoinSelector) CoinSelect(targetValue czzutil.Amount, coins []Coin) (Coins, error) {
...
// czzutil.Tx, a specific outpoint index, and the number of confirmations
...
func (c *SimpleCoin) Value() czzutil.Amount {
	return czzutil.Amount(c.txOut().Value)
}
